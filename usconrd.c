...
void prtaawcusn(aaw_c *aawc) /* print line and word details, but not the words themselves */
{
    int i, j;
    int len;
    for(i=0;i<aawc->numl;++i) {
        len=0;
        for(j=0;j<aawc->aaw[i]->al;++j)
            len += aawc->aaw[i]->aw[j]->lp1; // size of token will be included here
        len--; // take one off for new line.
        printf("N=%u/L=%u:", aawc->aaw[i]->al, len); 
        for(j=0;j<aawc->aaw[i]->al;++j)
            printf((j!=aawc->aaw[i]->al-1)?"%s(sz:%u)  ":"%s(sz:%u)\n", aawc->aaw[i]->aw[j]->w, aawc->aaw[i]->aw[j]->lp1-1);
    }
}
...
aaw_c *processinpfusn(char *fname) /* process underscore name file */
{
    /* declarations */
    FILE *fp=fopen(fname,"r");
    int i;
...
int main(int argc, char *argv[])
{
    ...
    aaw_c *aawc=processinpfusn(argv[1]);
    prtaawcusn(aawc);
